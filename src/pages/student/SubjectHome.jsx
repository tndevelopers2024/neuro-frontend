  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-16">
      ...
      {filteredCategories.length === 0 ? (
        <div className="bg-white border border-borderLine rounded-xl p-8 text-center shadow-soft max-w-2xl mx-auto my-12 animate-fadeIn">
          <div className="w-16 h-16 bg-[#FFF2F2] text-[#DC2626] rounded-lg flex items-center justify-center mx-auto mb-4 shadow-xs">
            <Search className="w-8 h-8 opacity-60" />
          </div>
          <h3 className="text-xl font-bold text-navy mb-2">No Matching Domains or Subtopics Found</h3>
          ...
          <button
            onClick={() => setSearchQuery('')}
            className="px-6 py-2.5 bg-primaryBlue text-white font-semibold text-xs rounded-xl shadow-md hover:bg-blue-600 transition-colors"
          >
            Clear Search Filter
          </button>
        </div>
          ...
          {filteredCategories.map((cat) => {
            ...
            return (
              <div
                ...
                className="bg-white border rounded-xl p-6 shadow-soft hover:shadow-elevated hover:-translate-y-1.5 transition-all duration-300 cursor-pointer flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div
                      style={{ backgroundColor: `${cat.color}15`, color: cat.color }}
                      className="w-14 h-14 rounded-lg flex items-center justify-center border border-current/20 group-hover:scale-110 transition-transform"
                    >
                  ...
                  <h3 className="text-lg font-bold text-navy uppercase tracking-tight group-hover:text-primaryBlue transition-colors">
                    {cat.name}
                  </h3>
                  ...
                  {cat.subtopics && cat.subtopics.length > 0 && (
                    <div className="space-y-1.5 mt-3 pt-3 border-t border-borderLine/60">
                      <div className="text-[11px] font-semibold text-navy uppercase tracking-wider flex items-center gap-1.5">
