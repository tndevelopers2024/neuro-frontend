...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn pb-16 max-w-7xl mx-auto">
      ...
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-navy to-[#123899] rounded-3xl p-8 md:p-10 text-white shadow-elevated flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="medical-card flex items-center justify-between">
          <div>
            ...
            <div className="text-3xl font-black text-primaryBlue mt-1">{stats.progressPercentage}%</div>
            <div className="text-xs font-semibold text-navy mt-1">{stats.topicsExplored} of {stats.totalTopics} Topics Explored</div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-[#E9F2FF] text-primaryBlue flex items-center justify-center">
        ...
        <div className="medical-card flex items-center justify-between">
          <div>
            ...
            <div className="text-3xl font-black text-navy mt-1">{stats.notesCreated}</div>
            <div className="text-xs font-semibold text-muted mt-1">Synchronized Note Syntheses</div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-secondaryBg text-navy flex items-center justify-center border border-borderLine">
        ...
        <div className="medical-card flex items-center justify-between">
          <div>
            ...
            <div className="text-3xl font-black text-medicalPurple mt-1">{stats.flashcardsAvailable}</div>
            <div className="text-xs font-semibold text-muted mt-1">High-Yield Board Flashcards</div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-[#F5EEFE] text-medicalPurple flex items-center justify-center">
        ...
        <div className="medical-card flex items-center justify-between">
          <div>
            ...
            <div className="text-3xl font-black text-medicalGreen mt-1">{stats.quizzesTaken}</div>
            <div className="text-xs font-semibold text-muted mt-1">Vignettes Evaluated</div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-[#EAF7ED] text-medicalGreen flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white border border-borderLine rounded-3xl p-7 shadow-soft">
          ...
          <div className="space-y-4">
            {activityList.map((item, idx) => (
              <Link
                key={idx}
                to={item.link || '/'}
                className="flex items-center justify-between p-4 rounded-2xl bg-secondaryBg border border-borderLine hover:bg-white hover:border-primaryBlue/30 hover:shadow-xs transition-all group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-extrabold text-sm text-navy group-hover:text-primaryBlue transition-colors">{item.title}</div>
        ...
        {/* Shortcuts Panel */}
        <div className="bg-white border border-borderLine rounded-3xl p-7 shadow-soft flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-extrabold text-navy mb-4 pb-3 border-b border-borderLine flex items-center gap-2">
